...
pub fn load_tables_from_odcs(
    input_path: &PathBuf,
) -> Result<Vec<data_modelling_core::models::Table>, CliError> {
    ...
    for table_data in import_result.tables {
        ...
        let columns: Vec<data_modelling_core::models::Column> = table_data
            .columns
            .iter()
            .map(|col_data| data_modelling_core::models::Column {
        ...
        tables.push(data_modelling_core::models::Table::new(table_name, columns));
    }
...
pub fn handle_export_odps(_args: &ExportArgs) -> Result<(), CliError> {
    ...
    #[cfg(feature = "odps-validation")]
    {
        ...
        // Import ODPS file
        use data_modelling_core::export::ODPSExporter;
    ...
    let pdf_result = match doc_type {
        Some("decision") => {
            ...
            exporter.export_decision(&decision).map_err(|e| {
                CliError::ExportError(data_modelling_core::export::ExportError::ExportError(
                    e.to_string(),
                ))
            })?
        }
