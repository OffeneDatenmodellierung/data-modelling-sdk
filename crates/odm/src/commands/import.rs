...
use crate::output::{collect_type_mappings, format_compact_output, format_pretty_output};
...
#[cfg(feature = "openapi")]
use data_modelling_core::validation::schema::validate_openapi_internal;
...
pub fn handle_import_sql(args: &ImportArgs) -> Result<(), CliError> {
    ...
    let mut result = importer.parse(&sql_content).map_err(|e| {
        CliError::ImportError(data_modelling_core::import::ImportError::ParseError(
            e.to_string(),
        ))
    })?;
...
pub fn handle_import_openapi(args: &ImportArgs) -> Result<(), CliError> {
    ...
    let spec: JsonValue = if openapi_content.trim_start().starts_with('{') {
        serde_json::from_str(&openapi_content).map_err(|e| {
            CliError::ImportError(data_modelling_core::import::ImportError::OpenAPIParseError(
                e.to_string(),
            ))
        })?
    } else {
    ...
    if let Some(schemas) = components.and_then(|s| s.as_object()) {
        for (schema_name, _schema_value) in schemas {
            match converter.convert_component(&openapi_content, schema_name, None) {
                Ok(table) => {
                    tables.push(data_modelling_core::import::TableData {
                        ...
                        columns: table
                            .columns
                            .iter()
                            .map(data_modelling_core::import::odcs_shared::column_to_column_data)
                            .collect(),
                        ..Default::default()
                    });
                }
                Err(e) => {
                    errors.push(data_modelling_core::import::ImportError::OpenAPIParseError(
                        e.to_string(),
                    ));
                }
            }
