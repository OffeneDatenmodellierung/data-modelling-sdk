...
impl BrowserStorageBackend {
    ...
    async fn get_db(&self) -> Result<IdbDatabase, StorageError> {
        ...
        let open_request = idb_factory
            ...
            .map_err(|e| StorageError::BackendError(format!("Failed to open IndexedDB: {:?}", e)))?;
        ...
        let request = result
            ...
            .ok_or_else(|| StorageError::BackendError("Result is not IdbOpenDbRequest".to_string()))?;
        
        let db_result = request
            .result()
            .map_err(|e| StorageError::BackendError(format!("Failed to get database result: {:?}", e)))?;
        
        db_result
            .dyn_into::<IdbDatabase>()
            .map_err(|e| StorageError::BackendError(format!("Failed to convert to IdbDatabase: {:?}", e)))
    }
    ...
    fn get_local_storage(&self) -> Result<Storage, StorageError> {
        ...
        window
            .local_storage()
            .map_err(|e| StorageError::BackendError(format!("localStorage not available: {:?}", e)))?
...
impl StorageBackend for BrowserStorageBackend {
    async fn read_file(&self, path: &str) -> Result<Vec<u8>, StorageError> {
        ...
        let transaction = db
            ...
            .map_err(|e| StorageError::BackendError(format!("Failed to create transaction: {:?}", e)))?;
        
        let request = {
            let store = transaction
                .object_store(&self.store_name)
                .map_err(|e| StorageError::BackendError(format!("Failed to get object store: {:?}", e)))?;
        ...
        let result = wasm_bindgen_futures::JsFuture::from(
            js_sys::Promise::from(wasm_bindgen::JsValue::from(request))
        )
        .await
        .map_err(|e| StorageError::BackendError(format!("Failed to read from IndexedDB: {:?}", e)))?;
        ...
        if result.is_undefined() {
            ...
            let value = storage
                .get_item(path)
                .map_err(|e| StorageError::BackendError(format!("Failed to read from localStorage: {:?}", e)))?;
        ...
        if content.len() > 5 * 1024 * 1024 {
            ...
            
            {
                ...
                store
                    ...
                    .map_err(|e| StorageError::BackendError(format!("Failed to write to IndexedDB: {:?}", e)))?;
                // store is dropped here
            }
            
            // Now await transaction completion without holding store reference
            JsFuture::from(
                js_sys::Promise::from(wasm_bindgen::JsValue::from(transaction))
            )
            .await
            ...
            storage
                .set_item(path, &value)
                .map_err(|e| StorageError::BackendError(format!("Failed to write to localStorage: {:?}", e)))?;
        }
    ...
    async fn delete_file(&self, path: &str) -> Result<(), StorageError> {
        ...
        
        {
            ...
            store
                ...
                .map_err(|e| StorageError::BackendError(format!("Failed to delete from IndexedDB: {:?}", e)))?;
            // store is dropped here
        }
